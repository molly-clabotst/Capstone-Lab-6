...
def validate(city, state, countryCode):
    while True:
        # just in case there is no state
        if state.upper()=='N/A':
            state = ''
        # formating just in case there is a state 
        else:
            state+=' ,'
        ...
        # capitalizing on the error code stored in the response
        if data['cod'] != '404':
            break
...
def storeData(data):
    ...
    for hour in range(len(data['list'])):
        # There are 8 entries for a day so values are reset at that point
        if hour%8==0:
            weatherTimes=[]
